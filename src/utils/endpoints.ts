...
export interface ITransformEndpointsProps {
...
export const transformGatsbyEndpoints = ({
    endpoints,
    projectRoot,
    configDir,
    cacheDir,
    opts,
}: ITransformEndpointsProps): IGatsbyEndpoints => {
    const newResolutions = {} as IGatsbyEndpoints;
    if (fileExists(cacheDir)) rimraf.sync(cacheDir);
    for (const [endpoint, resolvedPath] of Object.entries(endpoints)) {
        if (endpoint && resolvedPath) {
            const targetTransformer = new TransformTargets(
                projectRoot,
                cacheDir,
                opts,
            );
            const relativePath = path.relative(projectRoot, configDir);
            const { cacheFile } = targetTransformer.transform({
                srcFile: resolvedPath,
                targetFileName: `gatsby-${endpoint}.js`,
                relativePath,
            });

            newResolutions[(endpoint as IConfigTypes)] = cacheFile;
            targetTransformer.start();
        }
    }
